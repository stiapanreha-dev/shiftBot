...
async def show_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        rank_service.check_and_update_rank(user.id, year, month)

        # Get current rank from PostgreSQL
        rank_info = sheets.get_employee_rank(user.id, year, month)
        if rank_info:
            current_rank = rank_info.get('Current Rank', 'Rookie')
        else:
            current_rank = "Rookie"
